...
function toDbPayload(data: PropertyFormValues) {
  ...
  return {
    ...
    short_description: data.short_description ?? null,
    type: data.type,
    structure_type: data.structure_type ?? "house",
    status: data.status,
    ...
    total_cent: data.total_cent ?? null,
    ...
    tags: normalizePropertyTags(data.tags ?? []),
...
const NULLABLE_STRING_KEYS = new Set([
  "description", "short_description", "facing", "zip_code",
...
export async function updateProperty(
  ...
  const safePayload = stripImmutablePropertyFields(updatePayload);
  safePayload.price_label = null;
  if (Object.prototype.hasOwnProperty.call(safePayload, "tags")) {
    safePayload.tags = normalizePropertyTags(
