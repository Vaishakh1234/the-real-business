...
import { AdminRelatedListingsSection } from "@/components/admin/properties/AdminRelatedListingsSection";
import {
...
export function PropertyDetailClient({
  ...
  const detailTags = Array.isArray(property.tags)
    ? property.tags.map((t) => String(t).trim()).filter((t) => t.length > 0)
    : [];
      ...
      : []),
    {
      ...
      value:
        property.total_cent != null
          ? `${Number(property.total_cent).toLocaleString("en-IN", { maximumFractionDigits: 4 })} cent`
          : null,
    },
    ...(structureKind === "house"
      ? [
      ...
      <div className="min-w-0 space-y-4 pt-4 pb-8 sm:space-y-6 sm:pt-5 sm:pb-10 lg:space-y-8 lg:pt-6">
        ...
        <section className={CARD_CLASS}>
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:gap-4">
            ...
            <div className="min-w-0 flex-1 space-y-2 sm:space-y-3">
              ...
              <p className="text-sm">
                <span className={FIELD_LABEL_CLASS}>Property ID: </span>
                <span className={`${VALUE_CLASS} font-mono tabular-nums`}>
                  {formatDetailValue(property.listing_ref)}
                </span>
              </p>
              <p className="text-sm">
              ...
              <div className="text-sm">
                <p className={cn(FIELD_LABEL_CLASS, "mb-2 sm:mb-2.5")}>
                  Tags
                  {detailTags.length > 0 ? (
                ...
                {detailTags.length > 0 ? (
                  <>
                    <div className="flex flex-wrap gap-1.5 sm:gap-2">
                      {detailTags.map((t, i) => (
                        <Badge
                          key={`${t}-${i}`}
                          variant="secondary"
                          className="max-w-full whitespace-normal break-words rounded-lg border border-amber-200/80 bg-amber-50/90 text-left text-xs font-medium text-amber-900 sm:text-sm"
                        >
                          {t}
                        </Badge>
                      ))}
                    ...
                    <p
                      className={cn(
                        "mt-2 text-xs leading-relaxed text-muted-foreground sm:text-sm",
                        VALUE_MUTED_CLASS,
                      )}
                    >
                      <span className={FIELD_LABEL_CLASS}>All tags: </span>
                      {detailTags.join(", ")}
                    </p>
                  </>
                ) : (
                  <p
                    className={cn("mt-0.5 text-muted-foreground", VALUE_CLASS)}
                  >
                    {EMPTY_PLACEHOLDER}
                  </p>
                )}
              ...
              <p className="text-sm">
                ...
                <span className="ml-1.5 text-base font-bold text-muted-foreground sm:text-lg">
                  /{" "}
        ...
        <section className={CARD_CLASS}>
          ...
          >
            ...
            Amenities, tags & highlights
          </ProfileSectionLabel>
          <div className="mt-2 space-y-3 sm:mt-3 sm:space-y-4">
            <div>
              <p className={cn("mb-2 sm:mb-2.5", FIELD_LABEL_CLASS)}>
                Amenities:{" "}
              </p>
            ...
            <div>
              ...
              {detailTags.length > 0 ? (
                <>
                  ...
                  <p
                    className={cn(
                      "mt-2 font-mono text-xs leading-relaxed text-muted-foreground sm:text-sm",
                    )}
                  >
                    {detailTags.join(", ")}
                  </p>
            ...
            <div>
              ...
              {property.highlights && property.highlights.length > 0 ? (
                <ul className={cn("list-inside list-disc", VALUE_CLASS)}>
        ...
        <section className={CARD_CLASS}>
          ...
          {descriptionHtml ? (
            <div
              className={cn("mt-0.5", PROPERTY_DESCRIPTION_HTML_CLASS)}
