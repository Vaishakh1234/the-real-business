...
import type { PropertyWithRelations, Category } from "@/types";
...
function propertyToFormValues(
  ...
  return {
    ...
    type: "sale",
    structure_type: p.structure_type === "plot" ? "plot" : "house",
    status: p.status,
    ...
    total_cent: p.total_cent != null ? Number(p.total_cent) : undefined,
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Total cent</TableHead>
            ...
            {loading ? (
              ...
              data.map((row) => (
                <TableRow key={row.id}>
                  ...
                  <TableCell>{structureLabel(row.structure_type)}</TableCell>
                  <TableCell className="tabular-nums text-muted-foreground text-sm">
                    {row.total_cent != null
                      ? Number(row.total_cent).toLocaleString("en-IN", {
